public class MovieDatabase {
    public static void main(String[] args) {
        ...
        System.out.println("Studios in which plays the 'actor cu 2 oscaruri'");
        studios2Oscars(studioDatabase);
        System.out.println("Movies in which plays an actor over 50");
        moviesWithActorOver50(studioDatabase);

    }

    public static void studioWithMoreThanTwoMovies(Studio[] studioDatabase){
        for(int i=0;i<studioDatabase.length;i++){
            if(studioDatabase[i].nrOfMovies()>2){
                System.out.println(studioDatabase[i].getNume());
            }
        }
    }
    public static void studios2Oscars(Studio[] studioDatabase){
        for(int i=0;i<studioDatabase.length;i++){
    ...
    public static void moviesWithActorOver50(Studio[] studioDatabase){
        for(int i=0;i< studioDatabase.length;i++){
            for(int j=0; j< studioDatabase[i].getFilme().length;j++){
                if(studioDatabase[i].filmul(j).actorAbove50Playing())
                    System.out.println(studioDatabase[i].filmul(j).getNume());
            }
        }
    }
}
